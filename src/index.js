...
function findAllPSiblings(where) {
    let searchElements = function (el) {
        let elements = el.children;
        let res = [];

        for (let i = 0; i < elements.length; i++) {
            if (elements[i].nextElementSibling && elements[i].nextElementSibling.tagName == 'P') {
                res.push(elements[i]);
            }
            if (elements[i].children.length > 0) {
                res = res.concat(searchElements(elements[i]));
            }
        }
...
function deleteTextNodesRecursive(where) {
    ...
    for (let i = 0; i < el.length; i++) {
        if (el[i].nodeType == Node.TEXT_NODE) {
            where.removeChild(el[i]);
            i--;
        } else if (el[i].childNodes.length > 0) {
            deleteTextNodesRecursive(el[i]);
        }
    }
}
...
function collectDOMStat(root) {
    ...
    let analizator = function (where, res) {
        let el = where.childNodes;
        ...
        for (let i = 0; i < el.length; i++) {
            if (el[i].nodeType == Node.TEXT_NODE) {
                ...
                if (el[i].tagName in res.tags) {
                    res.tags[el[i].tagName]++;
                } else {
                    res.tags[el[i].tagName] = 1;
                }
                for (let c = 0; c < el[i].classList.length; c++) {
                    if (el[i].classList[c] in res.classes) {
                        res.classes[el[i].classList[c]]++;
                    } else {
                        res.classes[el[i].classList[c]] = 1;
                    }
                }
    ...
    let observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            if (mutation.addedNodes.length > 0) {
                ...
                fn({
                    type: 'remove',
                    nodes: [].slice.apply(mutation.removedNodes)
                });
            }
        });
    });
